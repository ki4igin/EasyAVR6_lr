 ...
 * Нажимая кнопку PB6 можно поочередно просмотреть принятые байты на
 * семисегментных индикаторах, которые подключены к портам B и C. На крайнем 
 * левом индикаторе отображается номер просматриваемого байта, на двух правых
 *  - сам принятый байт.
...
{
    /**
     * PB4, PB5 - входы с PullUp (к ним подключены кнопки)
     * PD2...PD7 - входы с PullUp (к ним подключен контактный переключатель)
     */
    PORTB |= (1 << PB4) | (1 << PB5);
    PORTD = 0xFF;
